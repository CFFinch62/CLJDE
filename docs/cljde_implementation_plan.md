# CLJDE — Clojure IDE Implementation Plan

**Project codename:** CLJDE (placeholder — final name TBD)
...
CLJDE is a self-contained integrated development environment for the Clojure programming language, designed around the principle that everything needed for productive Clojure development should live in one unified workspace. Unlike the prevailing Clojure tooling culture (Emacs + CIDER, Vim + Conjure, or multi-plugin VS Code configurations), CLJDE treats the REPL-driven development model as a first-class part of the IDE rather than as an add-on.
...
CLJDE v1.0 is complete when a developer can:
...
When CLJDE spawns `lein repl :headless` or `clj -M:nrepl`, it does so via QProcess. We parse the port from the output and connect our nREPL client to it. This separation means CLJDE can also connect to REPLs launched externally.
...
Build these modules: `main.py`, `CLJDE/app.py`, `CLJDE/main_window.py`, `CLJDE/config/settings.py`, `CLJDE/config/theme.py`, `CLJDE/config/paths.py`, `CLJDE/ui/menubar.py`, `CLJDE/ui/toolbar.py`, `CLJDE/ui/status_bar.py`.
...
`CLJDE/editor/editor_widget.py`, `CLJDE/editor/clojure_highlighter.py`, `CLJDE/editor/paren_matcher.py`, `CLJDE/editor/line_numbers.py`, `CLJDE/editor/form_detector.py`.
...
`CLJDE/files/file_tree.py`, `CLJDE/files/tab_manager.py`, `CLJDE/files/file_ops.py`, `CLJDE/files/project.py`.
...
- Close CLJDE with open tabs, relaunch, see the same tabs reopened with cursor positions restored
...
`CLJDE/nrepl/bencode.py`, `CLJDE/nrepl/client.py`, `CLJDE/nrepl/socket_worker.py`, `CLJDE/nrepl/messages.py`, `CLJDE/nrepl/session.py`.
...
`CLJDE/repl/repl_pane.py`, `CLJDE/repl/output_view.py`, `CLJDE/repl/input_line.py`, plus wiring updates in `main_window.py`.
...
Eliminate manual nREPL connection. CLJDE can spawn an nREPL process in a project and auto-connect.
...
- `ReplProcessManager` — wraps QProcess to launch REPLs:
  ...
  - Handles graceful shutdown on CLJDE quit
...
`CLJDE/repl/process_manager.py`, `CLJDE/repl/port_detector.py`, updates to MainWindow and toolbar.
...
- Close CLJDE while REPL is running, verify the REPL process is terminated (no orphans)
- Kill the REPL process externally, verify CLJDE detects this and updates UI state without crashing
...
`CLJDE/namespace/ns_browser.py`, `CLJDE/namespace/ns_operations.py`, structural editing additions to `CLJDE/editor/editor_widget.py`, balance checker possibly as new `CLJDE/editor/balance_checker.py`.
...
`CLJDE/ui/preferences_dialog.py`, `CLJDE/ui/icons.py`, resource files, packaging scripts.
...
```
You are helping me build CLJDE, a Clojure IDE written in Python 3.11+ with PyQt6.
...
OBJECTIVE:
Create the CLJDE project structure and a working PyQt6 main window with empty panels
...
3. Python modules:
   ...
   - CLJDE/app.py: QApplication subclass or factory function, ~100 lines.
     Sets org/app name, loads theme, handles app-wide signals.
   - CLJDE/main_window.py: MainWindow class, ~300 lines. QMainWindow subclass.
   ...
   - CLJDE/config/settings.py: load/save JSON settings file. Auto-creates on first
     run with sensible defaults. ~150 lines.
   - CLJDE/config/theme.py: defines the amber/blue/dark palette and generates a
   ...
   - CLJDE/config/paths.py: determines platform config dir (XDG on Linux,
   ...
   - CLJDE/ui/menubar.py: constructs the menu bar. Menus: File (New, Open File,
   ...
   - CLJDE/ui/toolbar.py: main toolbar with stub buttons for common actions.
     ~150 lines.
   - CLJDE/ui/status_bar.py: status bar with three QLabel sections — file info,
...
FILES TO CREATE:
- CLJDE/editor/__init__.py
- CLJDE/editor/editor_widget.py (~250 lines)
- CLJDE/editor/clojure_highlighter.py (~200 lines)
- CLJDE/editor/paren_matcher.py (~150 lines)
- CLJDE/editor/line_numbers.py (~120 lines)
- CLJDE/editor/form_detector.py (~180 lines)
- tests/test_form_detector.py (pytest tests for form boundary detection)

FILES TO MODIFY:
- CLJDE/main_window.py: replace the "Editor Area" placeholder label with a single
...
CONTEXT:
Phases 1 and 2 produced a CLJDE with a styled main window and a single working
...
FILES TO CREATE:
- CLJDE/files/__init__.py
- CLJDE/files/file_tree.py (~200 lines)
- CLJDE/files/tab_manager.py (~220 lines)
- CLJDE/files/file_ops.py (~180 lines)
- CLJDE/files/project.py (~150 lines)
- tests/test_project.py

FILES TO MODIFY:
- CLJDE/main_window.py: replace the "File Tree" placeholder with FileTreeWidget;
...
- CLJDE/config/settings.py: add fields for last_project_path, recent_files list,
...
ACCEPTANCE TEST:
1. Launch CLJDE with no prior project
...
11. Quit CLJDE with two tabs open
...
CONTEXT:
Phases 1–3 produced a CLJDE with a styled main window, working editor with
...
FILES TO CREATE:
- CLJDE/nrepl/__init__.py
- CLJDE/nrepl/bencode.py (~150 lines)
- CLJDE/nrepl/messages.py (~150 lines)
- CLJDE/nrepl/socket_worker.py (~180 lines)
- CLJDE/nrepl/client.py (~250 lines)
- CLJDE/nrepl/session.py (~180 lines)
...
CONTEXT:
Phases 1–4 produced:
- A styled CLJDE main window with editor, file tree, tabs, project detection
- A fully tested nREPL client library in CLJDE/nrepl/ with bencode, message
...
FILES TO CREATE:
- CLJDE/repl/__init__.py
- CLJDE/repl/repl_pane.py (~280 lines)
- CLJDE/repl/output_view.py (~200 lines)
- CLJDE/repl/input_line.py (~180 lines)
- CLJDE/ui/connect_dialog.py (~100 lines) — manual host/port dialog

FILES TO MODIFY:
- CLJDE/main_window.py:
...
File-namespace detection helper (add to CLJDE/editor/form_detector.py or a new
CLJDE/namespace/ns_detector.py):
...
CONTEXT:
Phases 1–5 produced a CLJDE that can connect to an externally-running nREPL
server and evaluate forms from the editor. The user must currently start the
nREPL manually in a separate terminal.

OBJECTIVE:
Automate REPL startup. CLJDE spawns an nREPL process for the current project,
parses its port, and auto-connects. Add Start/Stop/Restart REPL controls.

FILES TO CREATE:
- CLJDE/repl/process_manager.py (~220 lines)
- CLJDE/repl/port_detector.py (~100 lines)
...
- CLJDE/ui/toolbar.py: add Start / Stop / Restart buttons with icons and
  enable-state logic
- CLJDE/config/settings.py: add fields for lein_command and clj_command
...
ACCEPTANCE TEST:
1. Launch CLJDE, open a Leiningen project
...
11. See CLJDE detect the crash; status updates; no UI crash
12. Close CLJDE while REPL is running; verify no orphaned process
...
CONTEXT:
Phases 1–6 produced a CLJDE that can open projects, spawn or connect to nREPLs,
...
FILES TO CREATE:
- CLJDE/editor/balance_checker.py (~150 lines)
- CLJDE/editor/rainbow_parens.py (~130 lines)
- CLJDE/editor/structural_edit.py (~220 lines)
- CLJDE/namespace/__init__.py
- CLJDE/namespace/ns_browser.py (~180 lines)
- CLJDE/namespace/ns_operations.py (~150 lines)
- CLJDE/ui/fuzzy_ns_dialog.py (~120 lines)

FILES TO MODIFY:
- CLJDE/editor/editor_widget.py:
...
- CLJDE/config/settings.py: add rainbow_parens: bool field
...
ACCEPTANCE TEST:
1. Launch CLJDE, open a project, start REPL
...
FILES TO CREATE:
- CLJDE/ui/preferences_dialog.py (~250 lines)
- CLJDE/ui/about_dialog.py (~100 lines)
- CLJDE/ui/shortcuts_dialog.py (~120 lines)
- CLJDE/ui/icons.py (~80 lines)
- CLJDE/resources/icons/*.svg (a curated set)
- CLJDE/resources/themes/default.json (finalized)
- build/CLJDE.spec (PyInstaller spec)
...
FILES TO MODIFY:
- CLJDE/main_window.py: wire Preferences, About, Shortcuts menu items;
  apply preference changes live where possible
- CLJDE/config/settings.py: document and organize settings sections
- CLJDE/app.py: add splash screen
...
Icons to ship (in resources/icons/, SVG preferred):
  app.svg              (CLJDE lambda/paren mark, amber on dark)
...
- CLJDE.spec: PyInstaller spec, onefile binary, includes resources/
- build_linux.sh: script that sets up venv, installs deps, runs pyinstaller,
  puts output in dist/CLJDE
...
ACCEPTANCE TEST:
1. Launch CLJDE; splash screen shows briefly, then main window
...
10. Run build/build_linux.sh; dist/CLJDE executable appears
11. Copy dist/CLJDE to another user's home dir on the same machine; it runs
...
*End of CLJDE Implementation Plan v1.0*
